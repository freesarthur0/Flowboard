...
export async function sbFetch(path, method = 'GET', body = null, silent = false, _retries = 0) {
  setSyncing(true);
  try {
    ...
    const res = await fetch(window.SUPA_URL + '/rest/v1/' + path, {
      method, headers,
    ...
    if (!silent) {
      ...
      if (window.toast) window.toast('Erro Sync: ' + msg.substring(0, 100), '#ef4444');
    }
    throw e;
  }
}

export function setSyncing(active, error = false) {
...
if (typeof window !== 'undefined') {
  Object.assign(window, { sbFetch, setSyncing });
}
